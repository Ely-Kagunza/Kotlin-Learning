...
fun main() {
    ...
    println("Sum of discounted prices $discountedSum\n")
    ...
    val mostExpensive = menuItems.map { (_, price) -> price }.max() ?: 0.0
    val expensive = menuItems.values.maxOrNull() ?: 0.0
    println("The most expensive item in the menu is priced at KSh $expensive\n (Same output as $mostExpensive)\n")
    ...
    println("Average price is for the menuItems is: $averagePrice\n")
    // TODO 9: Create a set of order IDs (test with duplicates)
    val orderIds = setOf<String>("ID-1", "ID-2", "ID-3", "ID-4", "ID-5")
